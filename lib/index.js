	...
	|| [patchNode(parent, newNode, oldNode, index)].map(({el, changes}) =>
		[].concat(
			...
			(newNode && newNode.children || oldNode && oldNode.children || [])
				? updateChildren(el, newNode && newNode.children || [], oldNode && oldNode.children || [])
				: []
		)
	).pop();
...
const patch = (vdom, tree) => {
	...
	let patches = (vdom instanceof HTMLElement || typeof vdom === 'string')
		? updateChildren(el, tree.children, [])
		: updateChildren(vdom.el, tree.children, vdom.tree.children);
	console.log(patches);
	patches.forEach(p => p());
	return ({
		el,
